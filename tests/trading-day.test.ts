...
import {
  ETF_FLOW_RETRY_BUDGET_MS,
  ETF_FLOW_SOURCE_TIMEOUT_MS,
  buildETFItem,
  etfFlowsSource,
  formatMillion,
  formatTradingDate,
...
describe("ETF flow retry budget", () => {
  it("fits inside the source deadline with cleanup headroom", () => {
    expect(etfFlowsSource.timeoutMs).toBe(ETF_FLOW_SOURCE_TIMEOUT_MS);
    expect(ETF_FLOW_RETRY_BUDGET_MS).toBeLessThan(ETF_FLOW_SOURCE_TIMEOUT_MS);
  });
});
...
describe("parseETFTable", () => {
  ...
  it("selects the newest row at or before the requested trading date", () => {
    const result = parseETFTable(
      table(`
        <tr><td>5 Jan 2026</td><td>100.5</td><td>(20.5)</td><td>80</td></tr>
        <tr><td>6 Jan 2026</td><td>200</td><td>50</td><td>250</td></tr>
      `),
  ...
  it("throws instead of reporting zero when no valid flows are parsed", () => {
    expect(() =>
      parseETFTable(
        table(`<tr><td>5 Jan 2026</td><td>-</td><td>-</td><td>-</td></tr>`),
        new Date(Date.UTC(2026, 0, 5)),
      ),
    ).toThrow("No valid ETF flows found");
  });
});
